...
public class PaymentDomainServiceImpl implements PaymentDomainService {
    ...
    public PaymentEvent validateAndInitiatePayment(Payment payment,
        ...
        if (failureMessages.isEmpty()) {
            ...
            return new PaymentCompletedEvent(payment, ZonedDateTime.now(ZoneId.of(TIMEZONE)),
                    paymentCompletedEventDomainEventPublisher);
        } else {
            log.info("Payment initiation is failed for order id: {}", payment.getOrderId().getValue());
            payment.updateStatus(PaymentStatus.FAILED);
            return new PaymentFailedEvent(payment, ZonedDateTime.now(ZoneId.of(TIMEZONE)), failureMessages,
                    paymentFailedEventDomainEventPublisher);
        }
    }
    ...
    public PaymentEvent validateAndCancelPayment(Payment payment,
        ...
        if (failureMessages.isEmpty()) {
            ...
            return new PaymentCancelledEvent(payment, ZonedDateTime.now(ZoneId.of(TIMEZONE)),
                    paymentCancelledEventDomainEventPublisher);
        } else {
    ...
    private void updateCreditHistory(Payment payment,
        ...
        creditHistories.add(CreditHistory.Builder.builder()
                .setCreditHistoryId(new CreditHistoryId(UUID.randomUUID()))
